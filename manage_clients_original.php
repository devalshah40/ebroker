...
switch($_GET['action'])
{
	...
	case 'view':
		...
		{	
			...
			{
				$sql="";
				$sql = "INSERT INTO `clients` (`broker_id`, `name`, `address`, `year`, `brokerage`) VALUES ('".mysql_real_escape_string($_SESSION['login_id'])."','".ucwords(strtolower(mysql_real_escape_string($_POST['NAME'])))."','".mysql_real_escape_string($_POST['ADDRESS'])."','".mysql_real_escape_string($_POST['YEAR'])."','".mysql_real_escape_string($_POST['BROKERAGE'])."');";
				if(!mysql_query($sql))
				{
					$err = "Rows Not Inserted.";
				}
...
?>
<hr class="noscreen" />
		<form name="CLIENTFORM" action="manage_clients.php?action=<?php echo $frm_action; ?>" id="clientform" method="post" onsubmit="return formValidator(;">
		...
		<div id="content" class="box">
			...
			<table width="70%">
				<tr>
				    <th width="5%">CHECK<br/><input type='checkbox' onclick="checkAll()"></th>
				    <th width="18%">NAME</th>
			...
			<fieldset>
				...
				<table class="nostyle">
					...
					<tr>
						...
						<td><textarea cols="37" rows="7" name="ADDRESS" id="address" class="input-text"><?php if($edit_rec['address']!=""){ echo $edit_rec['address']; }else{ echo $_SESSION['ADDRESS']; } ?></textarea>
					...
					<tr>
						...
						<form name="CLIENTFORM1" action="manage_clients.php" id="clientform1" method="post">
						<head></head>
						<td class="t-right"><input type="submit" name="CANCEL" id="CANCEL" class="input-submit" value="Cancel"></td>
						</form>
					</tr>
				</table>
			</fieldset>
